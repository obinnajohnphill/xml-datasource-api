...
final class Reflection
{
	 ...
	 */
	public static function getReturnType(\ReflectionFunctionAbstract $func): ?ReflectionType
	{
		$type = $func->getReturnType() ?? (PHP_VERSION_ID >= 80100 ? $func->getTentativeReturnType() : null);
		return self::getTypeReflection($func, $type);
	}
	 ...
	 */
	public static function getParameterType(\ReflectionParameter $param): ?ReflectionType
	{
		return self::getTypeReflection($param, $param->getType());
	}
	 ...
	 */
	public static function getPropertyType(\ReflectionProperty $prop): ?ReflectionType
	{
		return self::getTypeReflection($prop, $prop->getType());
	}


	private static function getTypeReflection(
		...
		if ($type === null) {
			...
			return new ReflectionType(
				...
				. self::normalizeType($type->getName(), $reflection)
			);

		} elseif ($type instanceof \ReflectionUnionType || $type instanceof \ReflectionIntersectionType) {
			$names = array_map(
				fn($t) => self::normalizeType($t->getName(), $reflection),
				$type->getTypes(),
			);
			return new ReflectionType(implode($type instanceof \ReflectionUnionType ? '|' : '&', $names));
